...
Matrix::Matrix(int rows, int cols)
{
    if(rows<1||cols<1)
    {
        printf("Nieprawidlowa wartosc wierszy lub kolumn w czasie wykonywania konstruktora. Przerywanie pracy programu...\n");
        exit(-1);
    }
    r = rows;
    c = cols;
...
Matrix* Matrix::add(Matrix m2)
{
    if(rows()!=m2.rows()||cols()!=m2.cols())
    {
        ...
        cout << "["<<rows()<<","<<cols()<<"] != ["<<m2.rows()<<","<<m2.cols()<<"]" << endl;
        return NULL;
    }
